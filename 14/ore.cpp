...
int main(int argc, char * argv[]) {
  ...
  map<string,int> required_materials;
  required_materials["FUEL"] = 1;

  for (string mat : order) {
    int req = required_materials[mat];
    int amo = reactions[mat].first;
    int num_reactions = (req + amo - 1) / amo;
  ...
  cout << required_materials["ORE"] << endl;
}
