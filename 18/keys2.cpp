...
int main(int argc, char * argv[]) {
  ...
  for (int y = 0; y < h; y++) {
    for (int x = 0; x < w; x++) {
      if (('a' <= area[y][x] && area[y][x] <= 'z')
            || area[y][x] == '!' || area[y][x] == '@' || area[y][x] == '$' || area[y][x] == '%'
         ) {
        keys[area[y][x]] = {y,x};
      }
    }
  ...
  int num_keys = all_moves.size() - 4;
  cout << all_moves.size() << endl;

  int best_score;
  int cur_inv_score = 0;

  map<pair<set<char>,tuple<char,char,char,char>>,int> costs;
  priority_queue<tuple<int,set<char>,tuple<char,char,char,char>>, vector<tuple<int,set<char>,tuple<char,char,char,char>>>, greater<tuple<int,set<char>,tuple<char,char,char,char>>>> pq;
  pq.push({0,{},{'!','@','$','%'}});
  while (!pq.empty()) {
    tuple<int,set<char>,tuple<char,char,char,char>> p = pq.top();
    pq.pop();
    int cur_score = get<0>(p);
    set<char> s = get<1>(p);
    tuple<char,char,char,char> cs = get<2>(p);
    ...
    if (costs.find({s,cs}) != costs.end()) continue;
    costs[{s,cs}] = cur_score;

    // for (auto x : s) {
    //   cout << x;
    // }
    // cout << ",(" << get<0>(cs) << ',' << get<1>(cs) << ',' << get<2>(cs) << ',' << get<3>(cs) << ")," << cur_score << endl;
    ...
    for (auto a : all_moves[c]) {
      ...
      set<char> news = s;
      news.insert(d);
      tuple<char,char,char,char> newcs = cs;
      get<0>(newcs) = d;
      if (costs.find({news,newcs}) != costs.end() && costs[{news,newcs}] <= cur_score + dist) continue;
      pq.push({cur_score + dist, news, newcs});
    }
