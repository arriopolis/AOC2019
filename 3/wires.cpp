...
int main(int argc, char * argv[]) {
  ...
  for (auto p1 : wires[0]) {
    ...
    for (auto p2 : wires[1]) {
      ...
      int dx1 = newx1 - prevx1, dx2 = newx2 - prevx2;
      int dy1 = newy1 - prevy1, dy2 = newy2 - prevy2;
      ...
      if (dx1 == 0 && dx2 == 0) {
        ...
        if (prevx1 == prevx2)
        {
          for (int y = max(min(prevy1,newy1),min(prevy2,newy2)); y <= min(max(prevy1,newy1),max(prevy2,newy2)); y++) {
            intersections.push_back({abs(y)+abs(prevx1),prevx1,y});
            cout << prevx1 << ' ' << y << endl;
          }
        }
      } else if (dy1 == 0 && dy2 == 0) {
        ...
        if (prevy1 == prevy2)
        {
          for (int x = max(min(prevx1,newx1),min(prevx2,newx2)); x <= min(max(prevx1,newx1),max(prevx2,newx2)); x++) {
            intersections.push_back({abs(x)+abs(prevy1),x,prevy1});
            cout << x << ' ' << prevy1 << endl;
          }
        }
      } else if (dx1 == 0) {
        ...
        if (y <= max(prevy1, newy1) && y >= min(prevy1, newy1) && x <= max(prevx2, newx2) && x >= min(prevx2, newx2)) {
          intersections.push_back({abs(x)+abs(y),x,y});
          cout << x << ' ' << y << endl;
        }
      } else if (dx2 == 0) {
